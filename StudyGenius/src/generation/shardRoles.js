...
Ogni esercizio presente nel materiale deve essere risolto integralmente: Consegna $\\to$ Dati $\\to$ Schema Mentale ("Perché questa formula?") $\\to$ Calcoli completi $\\to$ Risultato con unità $\\to$ Controlli di Coerenza (dimensionale, segno). Includi anche un esercizio inverso o di correzione errore.`
  ...
  SYNTHESIS_AND_ORAL: {
    ...
    name: 'Formulario Ragionato, Mappa Concettuale SVG & Preparazione all\'Orale',
    directive: `🎯 MISSIONE DIDATTICA DELLO SHARD: FORMULARIO RAGIONATO, MAPPA CONCETTUALE SVG & DIFESA ORALE.
Costruisci la tabella sinottica delle formule con significato, condizioni e attenzioni.
MAPPA CONCETTUALE VETTORIALE OBBLIGATORIA: Genera una mappa concettuale completa di sintesi del capitolo in blocco vettoriale \`\`\`svg\\n<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 850 420" width="100%" height="100%">...</svg>\\n\`\`\` (oppure \`\`\`json:visual-spec\`). VIETATI ASSOLUTAMENTE alberi in caratteri ASCII (├──, └──).
Genera le domande di ragionamento tipiche del colloquio d'esame con le relative risposte argomentate e la checklist di padronanza.`
  }
};
