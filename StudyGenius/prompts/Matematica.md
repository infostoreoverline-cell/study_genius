...
## 📐 GRAFICA MATEMATICA E MAPPE CONCETTUALI IN SVG VETTORIALE PURO (ZERO ALBERI ASCII)

In Matematica (Analisi, Geometria, Algebra Lineare, Probabilità):
1. **Grafici di Funzione e Interpretazione Geometrica**:
   - Andamento qualitativo di funzioni $f(x)$, punti critici (massimi, minimi, flessi), asintoti verticali/orizzontali/obliqui, tangenti geometriche e significato della derivata $f'(x_0)$.
   - Significato dell'integrale definito come area orientata sottesa alla curva $y = f(x)$ tra gli estremi $[a, b]$.
2. **Diagrammi Insiemistici e Spazi Vettoriali**:
   - Diagrammi di Eulero-Venn per domini di definizione, insiemi aperti/chiusi, intorni sferici $B_r(x_0)$.
   - Vettori nel piano e nello spazio cartesiano $\mathbb{R}^2, \mathbb{R}^3$, proiezioni ortogonali, combinazioni lineari, piano generato da una base.
3. **Mappe Logiche di Dimostrazione e Alberi Decisionali**:
   - Flussi decisionali per lo studio di convergenza di serie numeriche o integrali impropri (confronto, radice, rapporto, Leibniz).
4. **FORMATO DI RENDERING OBBLIGATORIO**:
   Tutti i grafici, mappe e schemi DEVONO essere resi in codice SVG vettoriale puro:
   ````markdown
   ```svg
   <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ..."> ... </svg>
   ```
   ````
   oppure con specifica deterministica ````json:visual-spec````. È severamente vietato qualsiasi albero in caratteri ASCII (`├──`, `└──`).
