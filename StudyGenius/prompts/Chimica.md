...
## 🔬 SCHEMI VISUALI E IMPIANTISTICI IN SVG VETTORIALE PURO (ZERO ALBERI ASCII)

In Chimica Generale, Analitica e Industriale, la componente grafica e schematica è fondamentale per l'apprendimento e il superamento dell'esame:
1. **Schemi d'Impianto e Flusso (BFD, PFD, P&ID)**:
   - **BFD (Block Flow Diagram)**: blocchi funzionali (es. Reattore, Separatore, Colonna di Distillazione, Spurgo, Riciclo) con frecce orientate e indicazione chiara delle correnti di materia.
   - **PFD (Process Flow Diagram)**: apparecchiature industriali con icone geometriche SVG (reattore CSTR/PFR, scambiatori di calore, flash, colonne a piatti o a riempimento) e tabella sinottica delle correnti.
   - **P&ID (Piping and Instrumentation Diagram)**: linee di processo, anelli di controllo (sensori/trasmettitori es. TT, PT, FT, controller TC, PC, FC e valvole di controllo pneumatiche) con logiche di feedback o feedforward.
2. **Apparati di Laboratorio ed Elettrochimica**:
   - Celle galvaniche (Daniell) ed elettrolitiche: anodo, catodo, ponte salino, flusso di elettroni nel circuito esterno e migrazione ionica.
   - Linee di montaggio di laboratorio: beute, burette di titolazione, colonne cromatografiche.
3. **Mappe Concettuali e Meccanismi di Reazione**:
   - Mappe gerarchiche degli equilibri (es. albero decisionale del pH, equilibri simultanei acido-base e complessometria).
4. **FORMATO DI RENDERING OBBLIGATORIO**:
   Tutti questi schemi DEVONO essere resi come SVG vettoriali puri all'interno di blocchi markdown:
   ````markdown
   ```svg
   <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ..."> ... </svg>
   ```
   ````
   oppure con specifiche ````json:visual-spec````. È severamente vietato qualsiasi albero di caratteri ASCII (`├──`, `└──`).
