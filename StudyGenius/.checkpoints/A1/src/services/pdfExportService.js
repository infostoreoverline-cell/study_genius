...
function renderMarkdownOrHtmlWithMath(content, isMarkdown = true, knowledgeGraph = null) {
  ...
  if (isMarkdown) {
    ...
    let text = (content || '').replace(/```(?:json:scientific-figure|scientific-figure|json:plot|plot|json:graph|graph|json:figure|figure|svg|xml:svg|tikz|chemfig|circuitikz)[\s\S]*?```|\{\{GRAPH:[a-zA-Z0-9_\-]+\}\}/g, (block) => {
 ...
 */
async function generatePdf({ content, isMarkdown = true, title, subject, knowledgeGraph = null }) {
  ...
  try {
    const rendered = renderMarkdownOrHtmlWithMath(content, isMarkdown, knowledgeGraph);
