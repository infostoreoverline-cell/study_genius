...
function generateDidacticMarkdownDirective(evidence, strategy, consistencyState, conflicts) {
  ...
  if (strategy === RECONSTRUCTION_STRATEGIES.REDRAW_FROM_FORMULA_DATA) {
    ...
    strategyText = `Fai riferimento alla figura originale estratta dalla fonte: ![${caption}](${evidence.cropKey || figId})`;
  } else if (strategy === RECONSTRUCTION_STRATEGIES.RECONSTRUCT_CONCEPTUAL) {
    strategyText = `Genera uno schema concettuale rigoroso in blocco \`\`\`mermaid`;
  } else {
    strategyText = `Descrivi analiticamente la figura nel testo senza inventare coordinate esatte.`;
  }
