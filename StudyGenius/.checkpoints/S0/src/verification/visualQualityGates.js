...
/**
 * 3. DIDACTIC CRITIC
 * Valuta la funzione pedagogica, il protocollo ermeneutico e la riduzione del carico cognitivo
 */
class DidacticCritic {
  audit({ visualEvidences = [], markdownContent = '', visualCoverageMatrix = [] }) {
    const hardFails = [];
    const deficiencies = [];

    // Verifica presenza del protocollo ermeneutico per ciascun visuale integrato
    for (const item of visualEvidences) {
      const figId = item.figureId || item.id;
      if (!figId) continue;

      if (markdownContent.includes(figId)) {
        const figIdx = markdownContent.indexOf(figId);
        const surroundingText = markdownContent.slice(Math.max(0, figIdx - 400), Math.min(markdownContent.length, figIdx + 800));

        // Step 3: Lettura Guidata
...
class VisualQualityAuditor {
  ...
  auditPublication({
    ...
    // 3. Didactic Critic
    const didResult = this.didacticCritic.audit({ visualEvidences, markdownContent, visualCoverageMatrix });
